    ...
    include 'decrypt.php';
   	...
   	if( isset($_GET['id']) ){
   		...
   		if( file_exists("txt-db/$id.txt") ){ // check if the ID is has txt file in "txt-db" folder
   			$get_long_link = decrypt_param(file_get_contents("txt-db/$id.txt")); // get the long link from txt file
   			// GET THE ATTRIBUTES FROM THE $get_long_link

        // IFRAME-CONTENT SPLIT
        $iframe_string_content = explode('&iframe-content=', $get_long_link); 
        
        // TYPE SPLIT
        $iframe_string_type = explode('&iframe-type=', $iframe_string_content[0]); 

        // CTA SPLIT
        $iframe_string_cta = explode('&cta=', $iframe_string_type[0]); 

        // FACEBOOK-URL SPLIT
        $iframe_string_facebook_url = explode('&facebook-url=', $iframe_string_cta[0]); 

        // MESSAGE1 SPLIT
        $iframe_string_message1 = explode('&message1=', $iframe_string_facebook_url[0]); 

        // URL SPLIT
        $iframe_string_url = explode('url=', $iframe_string_message1[0]); 
        ...
        // echo '<br>';
			...
			$site_name = issetor($siteInformation['hybridGraph']['site_name'], '');
			$video = issetor($siteInformation['hybridGraph']['video'], '');  
			$locale = issetor($siteInformation['hybridGraph']['locale'], '');      
